...
let snakeBody = [{ x: 9, y: 9, head: 1, tail: 2 }];
let ssnakeBody = [
...
function snakeMoving(snakeDirection) {
  gameboard.innerHTML = "";
  let head = {
    x: snakeBody[snakeLength].x + snakeDirection.x,
    y: snakeBody[snakeLength].y + snakeDirection.y
  };
  if (
    head.x === 22 ||
    head.y === 22 ||
    head.x === 0 ||
    head.y === 0 ||
    snakeBody.some((el) => el.x === head.x && el.y === head.y)
  ) {
    snakeLength = 0;
    snakeBody = [];
...
function af(timeStamp) {
  ...
  const sslr = (timeStamp - lastTime) / 150;
  if (sslr <= 1) return;
  lastTime = timeStamp;
...
//might add multiplayer so stand by
